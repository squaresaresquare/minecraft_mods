...
public class TripleWindowCapLeft extends BlockWithEntity implements BlockEntityProvider {
    public static final MapCodec<TripleWindowCapLeft> CODEC = TripleWindowCapLeft.createCodec(TripleWindowCapLeft::new);
    public TripleWindowCapLeft(Settings settings) {
        super(settings);
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new TripleWindowCapLeftEntity(pos, state);
    }
    ...
    protected ActionResult onUseWithItem(ItemStack stack, BlockState state, World world, BlockPos pos,
        ...
        if(world.getBlockEntity(pos) instanceof TripleWindowCapLeftEntity TripleWindowCapLeftBlockEntity) {
            if(TripleWindowCapLeftBlockEntity.isEmpty() && !stack.isEmpty()) {
                TripleWindowCapLeftBlockEntity.setStack(0, stack.copyWithCount(1));
                world.playSound(player, pos, SoundEvents.ENTITY_ITEM_PICKUP, SoundCategory.BLOCKS, 1f, 2f);
                stack.decrement(1);

                TripleWindowCapLeftBlockEntity.markDirty();
                world.updateListeners(pos, state, state, 0);
            } else if(stack.isEmpty() && !player.isSneaking()) {
                ItemStack stackOnQuartzPillar = TripleWindowCapLeftBlockEntity.getStack(0);
                player.setStackInHand(Hand.MAIN_HAND, stackOnQuartzPillar);
                world.playSound(player, pos, SoundEvents.ENTITY_ITEM_PICKUP, SoundCategory.BLOCKS, 1f, 1f);
                TripleWindowCapLeftBlockEntity.clear();

                TripleWindowCapLeftBlockEntity.markDirty();
                world.updateListeners(pos, state, state, 0);
            } else if(player.isSneaking() && !world.isClient()) {
                player.openHandledScreen(TripleWindowCapLeftBlockEntity);
            }
        }
