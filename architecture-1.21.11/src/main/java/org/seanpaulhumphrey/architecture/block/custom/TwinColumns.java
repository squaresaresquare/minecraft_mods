...
public class TwinColumns extends BlockWithEntity implements BlockEntityProvider {
    public static final MapCodec<TwinColumns> CODEC = TwinColumns.createCodec(TwinColumns::new);
    public TwinColumns(Settings settings) {
        super(settings);
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new TwinColumnsEntity(pos, state);
    }
    ...
    protected ActionResult onUseWithItem(ItemStack stack, BlockState state, World world, BlockPos pos,
        ...
        if(world.getBlockEntity(pos) instanceof TwinColumnsEntity TwinColumnsBlockEntity) {
            if(TwinColumnsBlockEntity.isEmpty() && !stack.isEmpty()) {
                TwinColumnsBlockEntity.setStack(0, stack.copyWithCount(1));
                world.playSound(player, pos, SoundEvents.ENTITY_ITEM_PICKUP, SoundCategory.BLOCKS, 1f, 2f);
                stack.decrement(1);

                TwinColumnsBlockEntity.markDirty();
                world.updateListeners(pos, state, state, 0);
            } else if(stack.isEmpty() && !player.isSneaking()) {
                ItemStack stackOnQuartzPillar = TwinColumnsBlockEntity.getStack(0);
                player.setStackInHand(Hand.MAIN_HAND, stackOnQuartzPillar);
                world.playSound(player, pos, SoundEvents.ENTITY_ITEM_PICKUP, SoundCategory.BLOCKS, 1f, 1f);
                TwinColumnsBlockEntity.clear();

                TwinColumnsBlockEntity.markDirty();
                world.updateListeners(pos, state, state, 0);
            } else if(player.isSneaking() && !world.isClient()) {
                player.openHandledScreen(TwinColumnsBlockEntity);
            }
        }
