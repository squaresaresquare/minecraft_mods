...
public class TripleWindowMiddleMiddle extends BlockWithEntity implements BlockEntityProvider {
    public static final MapCodec<TripleWindowMiddleMiddle> CODEC = TripleWindowMiddleMiddle.createCodec(TripleWindowMiddleMiddle::new);
    public TripleWindowMiddleMiddle(Settings settings) {
        super(settings);
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new TripleWindowMiddleMiddleEntity(pos, state);
    }
    ...
    protected ActionResult onUseWithItem(ItemStack stack, BlockState state, World world, BlockPos pos,
        ...
        if(world.getBlockEntity(pos) instanceof TripleWindowMiddleMiddleEntity TripleWindowMiddleMiddleBlockEntity) {
            if(TripleWindowMiddleMiddleBlockEntity.isEmpty() && !stack.isEmpty()) {
                TripleWindowMiddleMiddleBlockEntity.setStack(0, stack.copyWithCount(1));
                world.playSound(player, pos, SoundEvents.ENTITY_ITEM_PICKUP, SoundCategory.BLOCKS, 1f, 2f);
                stack.decrement(1);

                TripleWindowMiddleMiddleBlockEntity.markDirty();
                world.updateListeners(pos, state, state, 0);
            } else if(stack.isEmpty() && !player.isSneaking()) {
                ItemStack stackOnQuartzPillar = TripleWindowMiddleMiddleBlockEntity.getStack(0);
                player.setStackInHand(Hand.MAIN_HAND, stackOnQuartzPillar);
                world.playSound(player, pos, SoundEvents.ENTITY_ITEM_PICKUP, SoundCategory.BLOCKS, 1f, 1f);
                TripleWindowMiddleMiddleBlockEntity.clear();

                TripleWindowMiddleMiddleBlockEntity.markDirty();
                world.updateListeners(pos, state, state, 0);
            } else if(player.isSneaking() && !world.isClient()) {
                player.openHandledScreen(TripleWindowMiddleMiddleBlockEntity);
            }
        }
