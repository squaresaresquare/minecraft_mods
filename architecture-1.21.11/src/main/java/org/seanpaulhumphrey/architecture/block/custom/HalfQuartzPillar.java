...
public class HalfQuartzPillar extends BlockWithEntity implements BlockEntityProvider {
    public static final MapCodec<HalfQuartzPillar> CODEC = HalfQuartzPillar.createCodec(HalfQuartzPillar::new);
    public HalfQuartzPillar(Settings settings) {
        super(settings);
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new HalfQuartzPillarEntity(pos, state);
    }
    ...
    protected ActionResult onUseWithItem(ItemStack stack, BlockState state, World world, BlockPos pos,
        ...
        if(world.getBlockEntity(pos) instanceof HalfQuartzPillarEntity HalfQuartzPillarBlockEntity) {
            if(HalfQuartzPillarBlockEntity.isEmpty() && !stack.isEmpty()) {
                HalfQuartzPillarBlockEntity.setStack(0, stack.copyWithCount(1));
                world.playSound(player, pos, SoundEvents.ENTITY_ITEM_PICKUP, SoundCategory.BLOCKS, 1f, 2f);
                stack.decrement(1);

                HalfQuartzPillarBlockEntity.markDirty();
                world.updateListeners(pos, state, state, 0);
            } else if(stack.isEmpty() && !player.isSneaking()) {
                ItemStack stackOnQuartzPillar = HalfQuartzPillarBlockEntity.getStack(0);
                player.setStackInHand(Hand.MAIN_HAND, stackOnQuartzPillar);
                world.playSound(player, pos, SoundEvents.ENTITY_ITEM_PICKUP, SoundCategory.BLOCKS, 1f, 1f);
                HalfQuartzPillarBlockEntity.clear();

                HalfQuartzPillarBlockEntity.markDirty();
                world.updateListeners(pos, state, state, 0);
            } else if(player.isSneaking() && !world.isClient()) {
                player.openHandledScreen(HalfQuartzPillarBlockEntity);
            }
        }
