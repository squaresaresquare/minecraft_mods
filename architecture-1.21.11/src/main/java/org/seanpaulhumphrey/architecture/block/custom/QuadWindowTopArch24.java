...
public class QuadWindowTopArch24 extends BlockWithEntity implements BlockEntityProvider {
    public static final MapCodec<QuadWindowTopArch24> CODEC = QuadWindowTopArch24.createCodec(QuadWindowTopArch24::new);
    public QuadWindowTopArch24(Settings settings) {
        super(settings);
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new QuadWindowTopArch24Entity(pos, state);
    }
    ...
    protected ActionResult onUseWithItem(ItemStack stack, BlockState state, World world, BlockPos pos,
        ...
        if(world.getBlockEntity(pos) instanceof QuadWindowTopArch24Entity QuadWindowTopArch24BlockEntity) {
            if(QuadWindowTopArch24BlockEntity.isEmpty() && !stack.isEmpty()) {
                QuadWindowTopArch24BlockEntity.setStack(0, stack.copyWithCount(1));
                world.playSound(player, pos, SoundEvents.ENTITY_ITEM_PICKUP, SoundCategory.BLOCKS, 1f, 2f);
                stack.decrement(1);

                QuadWindowTopArch24BlockEntity.markDirty();
                world.updateListeners(pos, state, state, 0);
            } else if(stack.isEmpty() && !player.isSneaking()) {
                ItemStack stackOnQuartzPillar = QuadWindowTopArch24BlockEntity.getStack(0);
                player.setStackInHand(Hand.MAIN_HAND, stackOnQuartzPillar);
                world.playSound(player, pos, SoundEvents.ENTITY_ITEM_PICKUP, SoundCategory.BLOCKS, 1f, 1f);
                QuadWindowTopArch24BlockEntity.clear();

                QuadWindowTopArch24BlockEntity.markDirty();
                world.updateListeners(pos, state, state, 0);
            } else if(player.isSneaking() && !world.isClient()) {
                player.openHandledScreen(QuadWindowTopArch24BlockEntity);
            }
        }
