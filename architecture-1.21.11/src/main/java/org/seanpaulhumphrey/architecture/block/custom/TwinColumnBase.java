...
public class TwinColumnBase extends BlockWithEntity implements BlockEntityProvider {
    public static final MapCodec<TwinColumnBase> CODEC = TwinColumnBase.createCodec(TwinColumnBase::new);
    public TwinColumnBase(Settings settings) {
        super(settings);
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new TwinColumnBaseEntity(pos, state);
    }
    ...
    protected ActionResult onUseWithItem(ItemStack stack, BlockState state, World world, BlockPos pos,
        ...
        if(world.getBlockEntity(pos) instanceof TwinColumnBaseEntity TwinColumnBaseBlockEntity) {
            if(TwinColumnBaseBlockEntity.isEmpty() && !stack.isEmpty()) {
                TwinColumnBaseBlockEntity.setStack(0, stack.copyWithCount(1));
                world.playSound(player, pos, SoundEvents.ENTITY_ITEM_PICKUP, SoundCategory.BLOCKS, 1f, 2f);
                stack.decrement(1);

                TwinColumnBaseBlockEntity.markDirty();
                world.updateListeners(pos, state, state, 0);
            } else if(stack.isEmpty() && !player.isSneaking()) {
                ItemStack stackOnQuartzPillar = TwinColumnBaseBlockEntity.getStack(0);
                player.setStackInHand(Hand.MAIN_HAND, stackOnQuartzPillar);
                world.playSound(player, pos, SoundEvents.ENTITY_ITEM_PICKUP, SoundCategory.BLOCKS, 1f, 1f);
                TwinColumnBaseBlockEntity.clear();

                TwinColumnBaseBlockEntity.markDirty();
                world.updateListeners(pos, state, state, 0);
            } else if(player.isSneaking() && !world.isClient()) {
                player.openHandledScreen(TwinColumnBaseBlockEntity);
            }
        }
