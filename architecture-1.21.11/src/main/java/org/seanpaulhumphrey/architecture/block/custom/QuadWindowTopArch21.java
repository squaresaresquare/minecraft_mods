...
public class QuadWindowTopArch21 extends BlockWithEntity implements BlockEntityProvider {
    public static final MapCodec<QuadWindowTopArch21> CODEC = QuadWindowTopArch21.createCodec(QuadWindowTopArch21::new);
    public QuadWindowTopArch21(Settings settings) {
        super(settings);
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new QuadWindowTopArch21Entity(pos, state);
    }
    ...
    protected ActionResult onUseWithItem(ItemStack stack, BlockState state, World world, BlockPos pos,
        ...
        if(world.getBlockEntity(pos) instanceof QuadWindowTopArch21Entity QuadWindowTopArch21BlockEntity) {
            if(QuadWindowTopArch21BlockEntity.isEmpty() && !stack.isEmpty()) {
                QuadWindowTopArch21BlockEntity.setStack(0, stack.copyWithCount(1));
                world.playSound(player, pos, SoundEvents.ENTITY_ITEM_PICKUP, SoundCategory.BLOCKS, 1f, 2f);
                stack.decrement(1);

                QuadWindowTopArch21BlockEntity.markDirty();
                world.updateListeners(pos, state, state, 0);
            } else if(stack.isEmpty() && !player.isSneaking()) {
                ItemStack stackOnQuartzPillar = QuadWindowTopArch21BlockEntity.getStack(0);
                player.setStackInHand(Hand.MAIN_HAND, stackOnQuartzPillar);
                world.playSound(player, pos, SoundEvents.ENTITY_ITEM_PICKUP, SoundCategory.BLOCKS, 1f, 1f);
                QuadWindowTopArch21BlockEntity.clear();

                QuadWindowTopArch21BlockEntity.markDirty();
                world.updateListeners(pos, state, state, 0);
            } else if(player.isSneaking() && !world.isClient()) {
                player.openHandledScreen(QuadWindowTopArch21BlockEntity);
            }
        }
