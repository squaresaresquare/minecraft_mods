...
public class TripleWindowTopArch12 extends BlockWithEntity implements BlockEntityProvider {
    public static final MapCodec<TripleWindowTopArch12> CODEC = TripleWindowTopArch12.createCodec(TripleWindowTopArch12::new);
    public TripleWindowTopArch12(Settings settings) {
        super(settings);
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new TripleWindowTopArch12Entity(pos, state);
    }
    ...
    protected ActionResult onUseWithItem(ItemStack stack, BlockState state, World world, BlockPos pos,
        ...
        if(world.getBlockEntity(pos) instanceof TripleWindowTopArch12Entity TripleWindowTopArch12BlockEntity) {
            if(TripleWindowTopArch12BlockEntity.isEmpty() && !stack.isEmpty()) {
                TripleWindowTopArch12BlockEntity.setStack(0, stack.copyWithCount(1));
                world.playSound(player, pos, SoundEvents.ENTITY_ITEM_PICKUP, SoundCategory.BLOCKS, 1f, 2f);
                stack.decrement(1);

                TripleWindowTopArch12BlockEntity.markDirty();
                world.updateListeners(pos, state, state, 0);
            } else if(stack.isEmpty() && !player.isSneaking()) {
                ItemStack stackOnQuartzPillar = TripleWindowTopArch12BlockEntity.getStack(0);
                player.setStackInHand(Hand.MAIN_HAND, stackOnQuartzPillar);
                world.playSound(player, pos, SoundEvents.ENTITY_ITEM_PICKUP, SoundCategory.BLOCKS, 1f, 1f);
                TripleWindowTopArch12BlockEntity.clear();

                TripleWindowTopArch12BlockEntity.markDirty();
                world.updateListeners(pos, state, state, 0);
            } else if(player.isSneaking() && !world.isClient()) {
                player.openHandledScreen(TripleWindowTopArch12BlockEntity);
            }
        }
