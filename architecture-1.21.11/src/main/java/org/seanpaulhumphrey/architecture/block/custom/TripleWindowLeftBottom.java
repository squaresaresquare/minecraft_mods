...
public class TripleWindowLeftBottom extends BlockWithEntity implements BlockEntityProvider {
    public static final MapCodec<TripleWindowLeftBottom> CODEC = TripleWindowLeftBottom.createCodec(TripleWindowLeftBottom::new);
    public TripleWindowLeftBottom(Settings settings) {
        super(settings);
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new TripleWindowLeftBottomEntity(pos, state);
    }
    ...
    protected ActionResult onUseWithItem(ItemStack stack, BlockState state, World world, BlockPos pos,
        ...
        if(world.getBlockEntity(pos) instanceof TripleWindowLeftBottomEntity TripleWindowLeftBottomBlockEntity) {
            if(TripleWindowLeftBottomBlockEntity.isEmpty() && !stack.isEmpty()) {
                TripleWindowLeftBottomBlockEntity.setStack(0, stack.copyWithCount(1));
                world.playSound(player, pos, SoundEvents.ENTITY_ITEM_PICKUP, SoundCategory.BLOCKS, 1f, 2f);
                stack.decrement(1);

                TripleWindowLeftBottomBlockEntity.markDirty();
                world.updateListeners(pos, state, state, 0);
            } else if(stack.isEmpty() && !player.isSneaking()) {
                ItemStack stackOnQuartzPillar = TripleWindowLeftBottomBlockEntity.getStack(0);
                player.setStackInHand(Hand.MAIN_HAND, stackOnQuartzPillar);
                world.playSound(player, pos, SoundEvents.ENTITY_ITEM_PICKUP, SoundCategory.BLOCKS, 1f, 1f);
                TripleWindowLeftBottomBlockEntity.clear();

                TripleWindowLeftBottomBlockEntity.markDirty();
                world.updateListeners(pos, state, state, 0);
            } else if(player.isSneaking() && !world.isClient()) {
                player.openHandledScreen(TripleWindowLeftBottomBlockEntity);
            }
        }
