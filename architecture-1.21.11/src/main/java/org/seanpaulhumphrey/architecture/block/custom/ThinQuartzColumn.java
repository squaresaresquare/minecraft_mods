...
public class ThinQuartzColumn extends BlockWithEntity implements BlockEntityProvider {
    public static final MapCodec<ThinQuartzColumn> CODEC = ThinQuartzColumn.createCodec(ThinQuartzColumn::new);
    public ThinQuartzColumn(Settings settings) {
        super(settings);
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new ThinQuartzColumnEntity(pos, state);
    }
    ...
    protected ActionResult onUseWithItem(ItemStack stack, BlockState state, World world, BlockPos pos,
        ...
        if(world.getBlockEntity(pos) instanceof ThinQuartzColumnEntity ThinQuartzColumnBlockEntity) {
            if(ThinQuartzColumnBlockEntity.isEmpty() && !stack.isEmpty()) {
                ThinQuartzColumnBlockEntity.setStack(0, stack.copyWithCount(1));
                world.playSound(player, pos, SoundEvents.ENTITY_ITEM_PICKUP, SoundCategory.BLOCKS, 1f, 2f);
                stack.decrement(1);

                ThinQuartzColumnBlockEntity.markDirty();
                world.updateListeners(pos, state, state, 0);
            } else if(stack.isEmpty() && !player.isSneaking()) {
                ItemStack stackOnQuartzPillar = ThinQuartzColumnBlockEntity.getStack(0);
                player.setStackInHand(Hand.MAIN_HAND, stackOnQuartzPillar);
                world.playSound(player, pos, SoundEvents.ENTITY_ITEM_PICKUP, SoundCategory.BLOCKS, 1f, 1f);
                ThinQuartzColumnBlockEntity.clear();

                ThinQuartzColumnBlockEntity.markDirty();
                world.updateListeners(pos, state, state, 0);
            } else if(player.isSneaking() && !world.isClient()) {
                player.openHandledScreen(ThinQuartzColumnBlockEntity);
            }
        }
