...
public class TripleWindowTopArchMiddle extends BlockWithEntity implements BlockEntityProvider {
    public static final MapCodec<TripleWindowTopArchMiddle> CODEC = TripleWindowTopArchMiddle.createCodec(TripleWindowTopArchMiddle::new);
    public TripleWindowTopArchMiddle(Settings settings) {
        super(settings);
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new TripleWindowTopArchMiddleEntity(pos, state);
    }
    ...
    protected ActionResult onUseWithItem(ItemStack stack, BlockState state, World world, BlockPos pos,
        ...
        if(world.getBlockEntity(pos) instanceof TripleWindowTopArchMiddleEntity TripleWindowTopArchMiddleBlockEntity) {
            if(TripleWindowTopArchMiddleBlockEntity.isEmpty() && !stack.isEmpty()) {
                TripleWindowTopArchMiddleBlockEntity.setStack(0, stack.copyWithCount(1));
                world.playSound(player, pos, SoundEvents.ENTITY_ITEM_PICKUP, SoundCategory.BLOCKS, 1f, 2f);
                stack.decrement(1);

                TripleWindowTopArchMiddleBlockEntity.markDirty();
                world.updateListeners(pos, state, state, 0);
            } else if(stack.isEmpty() && !player.isSneaking()) {
                ItemStack stackOnQuartzPillar = TripleWindowTopArchMiddleBlockEntity.getStack(0);
                player.setStackInHand(Hand.MAIN_HAND, stackOnQuartzPillar);
                world.playSound(player, pos, SoundEvents.ENTITY_ITEM_PICKUP, SoundCategory.BLOCKS, 1f, 1f);
                TripleWindowTopArchMiddleBlockEntity.clear();

                TripleWindowTopArchMiddleBlockEntity.markDirty();
                world.updateListeners(pos, state, state, 0);
            } else if(player.isSneaking() && !world.isClient()) {
                player.openHandledScreen(TripleWindowTopArchMiddleBlockEntity);
            }
        }
