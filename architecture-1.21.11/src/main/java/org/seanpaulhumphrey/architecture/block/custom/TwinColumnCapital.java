...
public class TwinColumnCapital extends BlockWithEntity implements BlockEntityProvider {
    public static final MapCodec<TwinColumnCapital> CODEC = TwinColumnCapital.createCodec(TwinColumnCapital::new);
    public TwinColumnCapital(Settings settings) {
        super(settings);
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new TwinColumnCapitalEntity(pos, state);
    }
    ...
    protected ActionResult onUseWithItem(ItemStack stack, BlockState state, World world, BlockPos pos,
        ...
        if(world.getBlockEntity(pos) instanceof TwinColumnCapitalEntity TwinColumnCapitalBlockEntity) {
            if(TwinColumnCapitalBlockEntity.isEmpty() && !stack.isEmpty()) {
                TwinColumnCapitalBlockEntity.setStack(0, stack.copyWithCount(1));
                world.playSound(player, pos, SoundEvents.ENTITY_ITEM_PICKUP, SoundCategory.BLOCKS, 1f, 2f);
                stack.decrement(1);

                TwinColumnCapitalBlockEntity.markDirty();
                world.updateListeners(pos, state, state, 0);
            } else if(stack.isEmpty() && !player.isSneaking()) {
                ItemStack stackOnQuartzPillar = TwinColumnCapitalBlockEntity.getStack(0);
                player.setStackInHand(Hand.MAIN_HAND, stackOnQuartzPillar);
                world.playSound(player, pos, SoundEvents.ENTITY_ITEM_PICKUP, SoundCategory.BLOCKS, 1f, 1f);
                TwinColumnCapitalBlockEntity.clear();

                TwinColumnCapitalBlockEntity.markDirty();
                world.updateListeners(pos, state, state, 0);
            } else if(player.isSneaking() && !world.isClient()) {
                player.openHandledScreen(TwinColumnCapitalBlockEntity);
            }
        }
