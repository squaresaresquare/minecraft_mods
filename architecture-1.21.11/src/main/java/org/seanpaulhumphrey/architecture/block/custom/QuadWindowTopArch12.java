...
public class QuadWindowTopArch12 extends BlockWithEntity implements BlockEntityProvider {
    public static final MapCodec<QuadWindowTopArch12> CODEC = QuadWindowTopArch12.createCodec(QuadWindowTopArch12::new);
    public QuadWindowTopArch12(Settings settings) {
        super(settings);
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new QuadWindowTopArch12Entity(pos, state);
    }
    ...
    protected ActionResult onUseWithItem(ItemStack stack, BlockState state, World world, BlockPos pos,
        ...
        if(world.getBlockEntity(pos) instanceof QuadWindowTopArch12Entity QuadWindowTopArch12BlockEntity) {
            if(QuadWindowTopArch12BlockEntity.isEmpty() && !stack.isEmpty()) {
                QuadWindowTopArch12BlockEntity.setStack(0, stack.copyWithCount(1));
                world.playSound(player, pos, SoundEvents.ENTITY_ITEM_PICKUP, SoundCategory.BLOCKS, 1f, 2f);
                stack.decrement(1);

                QuadWindowTopArch12BlockEntity.markDirty();
                world.updateListeners(pos, state, state, 0);
            } else if(stack.isEmpty() && !player.isSneaking()) {
                ItemStack stackOnQuartzPillar = QuadWindowTopArch12BlockEntity.getStack(0);
                player.setStackInHand(Hand.MAIN_HAND, stackOnQuartzPillar);
                world.playSound(player, pos, SoundEvents.ENTITY_ITEM_PICKUP, SoundCategory.BLOCKS, 1f, 1f);
                QuadWindowTopArch12BlockEntity.clear();

                QuadWindowTopArch12BlockEntity.markDirty();
                world.updateListeners(pos, state, state, 0);
            } else if(player.isSneaking() && !world.isClient()) {
                player.openHandledScreen(QuadWindowTopArch12BlockEntity);
            }
        }
