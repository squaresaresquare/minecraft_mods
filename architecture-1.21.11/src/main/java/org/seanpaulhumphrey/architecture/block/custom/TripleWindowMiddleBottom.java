...
public class TripleWindowMiddleBottom extends BlockWithEntity implements BlockEntityProvider {
    public static final MapCodec<TripleWindowMiddleBottom> CODEC = TripleWindowMiddleBottom.createCodec(TripleWindowMiddleBottom::new);
    public TripleWindowMiddleBottom(Settings settings) {
        super(settings);
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new TripleWindowMiddleBottomEntity(pos, state);
    }
    ...
    protected ActionResult onUseWithItem(ItemStack stack, BlockState state, World world, BlockPos pos,
        ...
        if(world.getBlockEntity(pos) instanceof TripleWindowMiddleBottomEntity TripleWindowMiddleBottomBlockEntity) {
            if(TripleWindowMiddleBottomBlockEntity.isEmpty() && !stack.isEmpty()) {
                TripleWindowMiddleBottomBlockEntity.setStack(0, stack.copyWithCount(1));
                world.playSound(player, pos, SoundEvents.ENTITY_ITEM_PICKUP, SoundCategory.BLOCKS, 1f, 2f);
                stack.decrement(1);

                TripleWindowMiddleBottomBlockEntity.markDirty();
                world.updateListeners(pos, state, state, 0);
            } else if(stack.isEmpty() && !player.isSneaking()) {
                ItemStack stackOnQuartzPillar = TripleWindowMiddleBottomBlockEntity.getStack(0);
                player.setStackInHand(Hand.MAIN_HAND, stackOnQuartzPillar);
                world.playSound(player, pos, SoundEvents.ENTITY_ITEM_PICKUP, SoundCategory.BLOCKS, 1f, 1f);
                TripleWindowMiddleBottomBlockEntity.clear();

                TripleWindowMiddleBottomBlockEntity.markDirty();
                world.updateListeners(pos, state, state, 0);
            } else if(player.isSneaking() && !world.isClient()) {
                player.openHandledScreen(TripleWindowMiddleBottomBlockEntity);
            }
        }
