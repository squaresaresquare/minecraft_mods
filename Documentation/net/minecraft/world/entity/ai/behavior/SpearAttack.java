...
public class SpearAttack extends Behavior<PathfinderMob> {
	...
	private int getKineticWeaponUseDuration(final PathfinderMob mob) {
		return (Integer)Optional.ofNullable(mob.getMainHandItem().get(DataComponents.KINETIC_WEAPON)).map(KineticWeapon::computeDamageUseDuration).orElse(0);
	}
	...
	protected void tick(final ServerLevel level, final PathfinderMob mob, final long timestamp) {
		...
		Vec3 awayPos = (Vec3)mob.getBrain().getMemory(MemoryModuleType.SPEAR_CHARGE_POSITION).orElse(null);
		if (awayPos != null) {
