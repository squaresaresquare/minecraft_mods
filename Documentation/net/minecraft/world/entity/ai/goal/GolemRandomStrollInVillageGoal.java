...
public class GolemRandomStrollInVillageGoal extends RandomStrollGoal {
	...
	private SectionPos getRandomVillageSection() {
		ServerLevel level = (ServerLevel)this.mob.level();
		List<SectionPos> villageSections = (List<SectionPos>)SectionPos.cube(SectionPos.of(this.mob), 2)
	...
	private BlockPos getRandomPoiWithinSection(final SectionPos sectionPos) {
		...
		List<BlockPos> pois = (List<BlockPos>)poiManager.getInRange(poiType -> true, sectionPos.center(), 8, PoiManager.Occupancy.IS_OCCUPIED)
