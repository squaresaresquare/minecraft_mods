...
public class CreakingAi {
		...
		return ActivityData.create(
			...
			ImmutableList.of(
				StartAttacking.create(
					(level, mob) -> mob instanceof Creaking && mob.isActive(), (level, mob) -> mob.getBrain().getMemory(MemoryModuleType.NEAREST_VISIBLE_ATTACKABLE_PLAYER)
				),
	...
	private static boolean isAttackTargetStillReachable(final Creaking creaking, final LivingEntity target) {
		...
		return (Boolean)visibleAttackablePlayers.map(players -> target instanceof Player player && players.contains(player)).orElse(false);
	}
