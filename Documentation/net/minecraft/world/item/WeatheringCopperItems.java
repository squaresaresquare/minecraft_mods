...
public record WeatheringCopperItems(
	...
	public static WeatheringCopperItems create(final WeatheringCopperBlocks blocks, final Function<Block, Item> itemFactory) {
		return new WeatheringCopperItems(
			(Item)itemFactory.apply(blocks.unaffected()),
			(Item)itemFactory.apply(blocks.exposed()),
			(Item)itemFactory.apply(blocks.weathered()),
			(Item)itemFactory.apply(blocks.oxidized()),
			(Item)itemFactory.apply(blocks.waxed()),
			(Item)itemFactory.apply(blocks.waxedExposed()),
			(Item)itemFactory.apply(blocks.waxedWeathered()),
			(Item)itemFactory.apply(blocks.waxedOxidized())
		);
	}
