...
public final class ShapedRecipePattern {
	...
	public static final MapCodec<ShapedRecipePattern> MAP_CODEC = ShapedRecipePattern.Data.MAP_CODEC
		.flatXmap(
			...
			pattern -> (DataResult)pattern.data.map(DataResult::success).orElseGet(() -> DataResult.error(() -> "Cannot encode unpacked recipe"))
		);
	...
	private boolean matches(final CraftingInput input, final boolean xFlip) {
		for (int y = 0; y < this.height; y++) {
			for (int x = 0; x < this.width; x++) {
				...
				if (xFlip) {
					expected = (Optional<Ingredient>)this.ingredients.get(this.width - x - 1 + y * this.width);
				} else {
					expected = (Optional<Ingredient>)this.ingredients.get(x + y * this.width);
				}
