			...
			for (EntityHitResult hitResult : (Collection)ProjectileUtil.getHitEntitiesAlong(
				...
				if (!wasStabbed) {
					...
					boolean dealsDismount = this.dismountConditions.isPresent()
						&& ((KineticWeapon.Condition)this.dismountConditions.get()).test(ticksUsed, attackerSpeedProjection, relativeSpeed, actionFactor);
					boolean dealsKnockback = this.knockbackConditions.isPresent()
						&& ((KineticWeapon.Condition)this.knockbackConditions.get()).test(ticksUsed, attackerSpeedProjection, relativeSpeed, actionFactor);
					boolean dealsDamage = this.damageConditions.isPresent()
						&& ((KineticWeapon.Condition)this.damageConditions.get()).test(ticksUsed, attackerSpeedProjection, relativeSpeed, actionFactor);
