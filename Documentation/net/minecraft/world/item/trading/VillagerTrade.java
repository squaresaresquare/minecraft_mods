	...
	public MerchantOffer getOffer(final LootContext lootContext) {
		if (this.merchantPredicate.isPresent() && !((LootItemCondition)this.merchantPredicate.get()).test(lootContext)) {
			...
			if (this.doubleTradePriceEnchantments.isPresent()) {
				HolderSet<Enchantment> enchantments = (HolderSet<Enchantment>)this.doubleTradePriceEnchantments.get();
			...
			if (itemCost.count() < 1) {
				...
				return additionalItemCost.isPresent() && ((ItemCost)additionalItemCost.get()).count() < 1
					? null
					: new MerchantOffer(
						itemCost,
						additionalItemCost,
