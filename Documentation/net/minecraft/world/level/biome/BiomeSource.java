...
public abstract class BiomeSource implements BiomeResolver {
	...
	public Set<Holder<Biome>> getBiomesWithin(final int x, final int y, final int z, final int r, final Climate.Sampler sampler) {
		...
		int d = y1 - y0 + 1;
		int h = z1 - z0 + 1;
		Set<Holder<Biome>> biomeSet = Sets.<Holder<Biome>>newHashSet();
	...
	public Pair<BlockPos, Holder<Biome>> findClosestBiome3d(
		...
		Set<Holder<Biome>> candidateBiomes = (Set<Holder<Biome>>)this.possibleBiomes().stream().filter(allowed).collect(Collectors.toUnmodifiableSet());
