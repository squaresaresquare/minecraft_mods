...
public class StateDefinition<O, S extends StateHolder<O, S>> {
	...
	private static <O, S extends StateHolder<O, S>> ImmutableList<S> createMultiPropertyStates(
		...
		for (List<Comparable<?>> values : stateValues) {
			...
			S blockState = (S)factory.create(owner, propertyKeys, (Comparable<?>[])valuesCopy.toArray(EMPTY_VALUES));
	...
	private static <S extends StateHolder<?, S>, T extends Comparable<T>> MapCodec<S> appendPropertyCodec(
		...
		return Codec.mapPair(
				codec, property.valueCodec().fieldOf(name).orElseGet((Consumer<String>)(var0 -> {}), () -> property.value((StateHolder<?, ?>)defaultSupplier.get()))
			)
			.xmap(
