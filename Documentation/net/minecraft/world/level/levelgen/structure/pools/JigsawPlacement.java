...
public class JigsawPlacement {
	...
	public static Optional<Structure.GenerationStub> addPieces(
		...
		StructureTemplatePool centerPool = (StructureTemplatePool)startPool.unwrapKey()
		...
		if (centerElement == EmptyPoolElement.INSTANCE) {
			...
			int bottomY = projectStartToHeightmap.isEmpty()
				...
				: position.getY()
					+ chunkGenerator.getFirstFreeHeight(centerX, centerZ, (Heightmap.Types)projectStartToHeightmap.get(), heightAccessor, context.randomState());
			...
			if (isStartTooCloseToWorldHeightLimits(heightAccessor, dimensionPadding, centerPiece.getBoundingBox())) {
				...
				return Optional.of(
					new Structure.GenerationStub(
						...
						builder -> {
							List<PoolElementStructurePiece> pieces = Lists.<PoolElementStructurePiece>newArrayList();
				...
				if (maybeTargetPool.isEmpty()) {
					...
					if (targetPool.value().size() == 0 && !targetPool.is(Pools.EMPTY)) {
						...
						if (fallback.value().size() == 0 && !fallback.is(Pools.EMPTY)) {
							...
							List<StructurePoolElement> targetPieces = Lists.<StructurePoolElement>newArrayList();
