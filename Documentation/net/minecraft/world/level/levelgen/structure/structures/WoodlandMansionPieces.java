...
public class WoodlandMansionPieces {
	...
	private static class MansionGrid {
		...
		private void setupThirdFloor() {
			List<Tuple<Integer, Integer>> potentialRooms = Lists.<Tuple<Integer, Integer>>newArrayList();
			...
			if (potentialRooms.isEmpty()) {
				...
				Tuple<Integer, Integer> roomPos = (Tuple<Integer, Integer>)potentialRooms.get(this.random.nextInt(potentialRooms.size()));
				...
				List<Direction> potentialCorridors = Lists.<Direction>newArrayList();
	...
	private static class MansionPiecePlacer {
		...
		public void createMansion(
			...
			for (int floorNum = 0; floorNum < 3; floorNum++) {
				...
				List<Direction> doorDirs = Lists.<Direction>newArrayList();
					...
					for (int xxx = 0; xxx < grid.width; xxx++) {
						...
						if (grid.get(xxx, y) == 2 || thirdFloorStartRoom) {
							...
							if (grid.get(xxx + 1, y) == 1 && !thirdFloorStartRoom) {
								...
								pieces.add(
									new WoodlandMansionPieces.WoodlandMansionPiece(this.structureTemplateManager, doorDir == Direction.EAST ? doorPiece : wallPiece, posx, rotation)
								);
							}
							...
							if (WoodlandMansionPieces.MansionGrid.isHouse(grid, xxx, y + 1) && !mansion.isRoomId(grid, xxx, y + 1, floorNum, roomId)) {
								...
								pieces.add(
									new WoodlandMansionPieces.WoodlandMansionPiece(
										this.structureTemplateManager, doorDir == Direction.SOUTH ? doorPiece : wallPiece, posx, rotation.getRotated(Rotation.CLOCKWISE_90)
									)
								);
							}
							...
							if (grid.get(xxx, y - 1) == 1 && !thirdFloorStartRoom) {
								...
								pieces.add(
									new WoodlandMansionPieces.WoodlandMansionPiece(
										this.structureTemplateManager, doorDir == Direction.NORTH ? doorPiece : wallPiece, posx, rotation.getRotated(Rotation.CLOCKWISE_90)
									)
								);
							}
				...
				for (int xxx = 0; xxx < grid.width; xxx++) {
					...
					if (WoodlandMansionPieces.MansionGrid.isHouse(grid, xxx, y) && !isAbove) {
						...
						if (!WoodlandMansionPieces.MansionGrid.isHouse(grid, xxx - 1, y)) {
							...
							if (!WoodlandMansionPieces.MansionGrid.isHouse(grid, xxx, y - 1)) {
								pieces.add(
									new WoodlandMansionPieces.WoodlandMansionPiece(this.structureTemplateManager, "roof_corner", p2x, rotation.getRotated(Rotation.CLOCKWISE_180))
								);
							} else if (WoodlandMansionPieces.MansionGrid.isHouse(grid, xxx - 1, y - 1)) {
	...
	public static class WoodlandMansionPiece extends TemplateStructurePiece {
		...
		public WoodlandMansionPiece(final StructureTemplateManager structureTemplateManager, final CompoundTag tag) {
			super(
				...
				location -> makeSettings((Mirror)tag.read("Mi", Mirror.LEGACY_CODEC).orElseThrow(), (Rotation)tag.read("Rot", Rotation.LEGACY_CODEC).orElseThrow())
			);
		}
