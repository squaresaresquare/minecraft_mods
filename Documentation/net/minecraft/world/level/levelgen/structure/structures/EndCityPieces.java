...
public class EndCityPieces {
	...
	private static final EndCityPieces.SectionGenerator TOWER_GENERATOR = new EndCityPieces.SectionGenerator() {
		...
		public boolean generate(
			...
			EndCityPieces.EndCityPiece lastPiece = EndCityPieces.addHelper(
				pieces,
				EndCityPieces.addPiece(structureTemplateManager, parent, new BlockPos(3 + random.nextInt(2), -3, 3 + random.nextInt(2)), "tower_base", rotation, true)
			);
			lastPiece = EndCityPieces.addHelper(
				...
				for (Tuple<Rotation, BlockPos> bridge : EndCityPieces.TOWER_BRIDGES) {
					if (random.nextBoolean()) {
						EndCityPieces.EndCityPiece bridgeStart = EndCityPieces.addHelper(
							pieces, EndCityPieces.addPiece(structureTemplateManager, bridgePiece, bridge.getB(), "bridge_end", rotation.getRotated(bridge.getA()), true)
						);
	...
	private static boolean recursiveChildren(
		...
		if (genDepth > 8) {
			...
			List<StructurePiece> childPieces = Lists.<StructurePiece>newArrayList();
	...
	public static class EndCityPiece extends TemplateStructurePiece {
		...
		public EndCityPiece(final StructureTemplateManager structureTemplateManager, final CompoundTag tag) {
			super(
				...
				location -> makeSettings(tag.getBooleanOr("OW", false), (Rotation)tag.read("Rot", Rotation.LEGACY_CODEC).orElseThrow())
			);
		}
