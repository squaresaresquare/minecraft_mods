...
public class MapItemSavedData extends SavedData {
	...
	private MapItemSavedData(
		...
		for (MapBanner banner : banners) {
			...
			this.addDecoration(banner.getDecoration(), null, banner.getId(), banner.pos().getX(), banner.pos().getZ(), 180.0, (Component)banner.name().orElse(null));
		}
	...
	public void tickCarriedBy(final Player tickingPlayer, final ItemStack itemStack, @Nullable final ItemFrame placedInFrame) {
		...
		for (int i = 0; i < this.carriedBy.size(); i++) {
			MapItemSavedData.HoldingPlayer otherHoldingPlayer = (MapItemSavedData.HoldingPlayer)this.carriedBy.get(i);
	...
	public boolean toggleBanner(final LevelAccessor level, final BlockPos pos) {
		...
		if (xd >= -63.0 && yd >= -63.0 && xd <= 63.0 && yd <= 63.0) {
			...
			if (!this.isTrackedCountOverLimit(256)) {
				this.bannerMarkers.put(banner.getId(), banner);
				this.addDecoration(banner.getDecoration(), level, banner.getId(), xPos, zPos, 180.0, (Component)banner.name().orElse(null));
				this.setDirty();
				return true;
			}
		}
	...
	public record MapPatch(int startX, int startY, int width, int height, byte[] mapColors) {
		...
		private static void write(final ByteBuf output, final Optional<MapItemSavedData.MapPatch> optional) {
			if (optional.isPresent()) {
				MapItemSavedData.MapPatch patch = (MapItemSavedData.MapPatch)optional.get();
