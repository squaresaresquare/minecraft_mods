...
public abstract class ChunkGenerator {
	...
	public ChunkGenerator(final BiomeSource biomeSource, final Function<Holder<Biome>, BiomeGenerationSettings> generationSettingsGetter) {
		...
		this.featuresPerStep = Suppliers.memoize(
			() -> FeatureSorter.buildFeaturesPerStep(
				List.copyOf(biomeSource.possibleBiomes()), b -> ((BiomeGenerationSettings)generationSettingsGetter.apply(b)).features(), true
			)
		);
	}
				...
				for (Entry<StructurePlacement, Set<Holder<Structure>>> entry : placementScans.entrySet()) {
					...
					if (placement instanceof ConcentricRingsStructurePlacement rings) {
						Pair<BlockPos, Holder<Structure>> generating = this.getNearestGeneratedStructure(
							(Set<Holder<Structure>>)entry.getValue(), level, structureManager, pos, createReference, rings
						);
						if (generating != null) {
					...
					for (int radius = 0; radius <= maxSearchRadius; radius++) {
						...
						for (Entry<StructurePlacement, Set<Holder<Structure>>> entryx : randomSpreadEntries) {
							...
							Pair<BlockPos, Holder<Structure>> structurePos = getNearestGeneratedStructure(
								(Set<Holder<Structure>>)entryx.getValue(),
								level,
								structureManager,
								chunkOriginX,
								chunkOriginZ,
	...
	public void applyBiomeDecoration(final WorldGenLevel level, final ChunkAccess chunk, final StructureManager structureManager) {
		...
		if (!SharedConstants.debugVoidTerrain(centerPos)) {
			...
			Map<Integer, List<Structure>> structuresByStep = (Map<Integer, List<Structure>>)structuresRegistry.stream()
				.collect(Collectors.groupingBy(structure -> structure.step().ordinal()));
			List<FeatureSorter.StepFeatureData> featureList = (List<FeatureSorter.StepFeatureData>)this.featuresPerStep.get();
				...
				for (int stepIndex = 0; stepIndex < generationSteps; stepIndex++) {
					...
					if (structureManager.shouldGenerateStructures()) {
						for (Structure structure : (List)structuresByStep.getOrDefault(stepIndex, Collections.emptyList())) {
							...
							Supplier<String> currentlyGenerating = () -> (String)structuresRegistry.getResourceKey(structure).map(Object::toString).orElseGet(structure::toString);
					...
					if (stepIndex < featureStepCount) {
						...
						for (Holder<Biome> biome : possibleBiomes) {
							...
							if (stepIndex < featuresInBiome.size()) {
								HolderSet<PlacedFeature> featuresInBiomeThisStep = (HolderSet<PlacedFeature>)featuresInBiome.get(stepIndex);
						...
						for (int featureIndex = 0; featureIndex < numberOfFeaturesInStep; featureIndex++) {
							...
							Supplier<String> currentlyGenerating = () -> (String)featureRegistry.getResourceKey(feature).map(Object::toString).orElseGet(feature::toString);
						...
						if (featurePlacement.isStructureChunk(state, sourceChunkPos.x(), sourceChunkPos.z())) {
							if (structures.size() == 1) {
								this.tryGenerateStructure(
									(StructureSet.StructureSelectionEntry)structures.get(0),
									structureManager,
									registryAccess,
									randomState,
			...
			for (int sourceZ = targetZ - 8; sourceZ <= targetZ + 8; sourceZ++) {
				...
				for (StructureStart start : level.getChunk(sourceX, sourceZ).getAllStarts().values()) {
					try {
						...
						structure.setDetail(
							"Id", (CrashReportDetail<String>)(() -> (String)configuredStructuresRegistry.map(r -> r.getKey(start.getStructure()).toString()).orElse("UNKNOWN"))
						);
						structure.setDetail("Name", (CrashReportDetail<String>)(() -> BuiltInRegistries.STRUCTURE_TYPE.getKey(start.getStructure().type()).toString()));
						structure.setDetail("Class", (CrashReportDetail<String>)(() -> start.getStructure().getClass().getCanonicalName()));
						throw new ReportedException(report);
					}
				}
			}
