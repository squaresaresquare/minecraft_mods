...
public class LoomMenu extends AbstractContainerMenu {
	...
	public boolean clickMenuButton(final Player player, final int buttonId) {
		if (buttonId >= 0 && buttonId < this.selectablePatterns.size()) {
			...
			this.setupResultSlot((Holder<BannerPattern>)this.selectablePatterns.get(buttonId));
			return true;
		} else {
			return false;
		}
	...
	public void slotsChanged(final Container container) {
		...
		if (!bannerStack.isEmpty() && !dyeStack.isEmpty()) {
			...
			if (this.selectablePatterns.size() == 1) {
				...
				patternToDisplay = (Holder<BannerPattern>)this.selectablePatterns.get(0);
			} else if (!validPatternIndex) {
				this.selectedBannerPatternIndex.set(-1);
				patternToDisplay = null;
			} else {
				Holder<BannerPattern> selectedValue = (Holder<BannerPattern>)previousSelectablePatterns.get(selectedPattern);
