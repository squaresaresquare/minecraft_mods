...
public class PillagerOutpostPools {
	...
	public static void bootstrap(final BootstrapContext<StructureTemplatePool> context) {
				...
				ImmutableList.of(
					Pair.of(
						StructurePoolElement.list(
							ImmutableList.of(
								StructurePoolElement.legacy("pillager_outpost/watchtower"), StructurePoolElement.legacy("pillager_outpost/watchtower_overgrown", outpostRot)
							)
						),
						1
					)
		...
		Pools.register(
			...
			new StructureTemplatePool(
				empty, ImmutableList.of(Pair.of(StructurePoolElement.legacy("pillager_outpost/feature_plate"), 1)), StructureTemplatePool.Projection.TERRAIN_MATCHING
			)
		);
		Pools.register(
			context,
