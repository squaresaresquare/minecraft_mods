	...
	public void generate(final HolderLookup.Provider registries, final Consumer<AdvancementHolder> output) {
		...
		AdvancementHolder waxOn = Advancement.Builder.advancement()
			...
			.addCriterion(
				"wax_on",
				ItemUsedOnLocationTrigger.TriggerInstance.itemUsedOnBlock(
					LocationPredicate.Builder.location().setBlock(BlockPredicate.Builder.block().of(blocks, ((BiMap)HoneycombItem.WAXABLES.get()).keySet())),
		...
		Advancement.Builder.advancement()
			...
			.addCriterion(
				"wax_off",
				ItemUsedOnLocationTrigger.TriggerInstance.itemUsedOnBlock(
					LocationPredicate.Builder.location().setBlock(BlockPredicate.Builder.block().of(blocks, ((BiMap)HoneycombItem.WAX_OFF_BY_BLOCK.get()).keySet())),
						...
						.withComponents(
							DataComponentMatchers.Builder.components()
								.partial(
									...
									EnchantmentsPredicate.enchantments(List.of(new EnchantmentPredicate(enchantments.getOrThrow(Enchantments.SILK_TOUCH), MinMaxBounds.Ints.atLeast(1))))
								)
								.build()
						),
			...
			.addCriterion(
				...
				StartRidingTrigger.TriggerInstance.playerStartsRiding(
					EntityPredicate.Builder.entity()
						.vehicle(
							EntityPredicate.Builder.entity().of(entityTypes, EntityTypeTags.BOAT).passenger(EntityPredicate.Builder.entity().of(entityTypes, EntityType.GOAT))
						)
				)
			)
		...
		AdvancementHolder feedSnifflet = Advancement.Builder.advancement()
			...
			.addCriterion(
				...
				PlayerInteractTrigger.TriggerInstance.itemUsedOnEntity(
					...
					Optional.of(
						EntityPredicate.wrap(EntityPredicate.Builder.entity().of(entityTypes, EntityType.SNIFFER).flags(EntityFlagsPredicate.Builder.flags().setIsBaby(true)))
					)
				)
			)
						...
						Optional.of(
							EntityPredicate.wrap(
								EntityPredicate.Builder.entity()
									...
									.components(DataComponentMatchers.Builder.components().exact(DataComponentExactPredicate.expect(DataComponents.FROG_VARIANT, frogVariant)).build())
							)
						)
					)
				)
	...
	private static Advancement.Builder addBreedable(
		...
		breedable.forEach(
			animal -> advancement.addCriterion(
				EntityType.getKey(animal).toString(), BredAnimalsTrigger.TriggerInstance.bredAnimals(EntityPredicate.Builder.entity().of(entityTypes, animal))
			)
		);
		indirectlyBreedable.forEach(
			animal -> advancement.addCriterion(
				EntityType.getKey(animal).toString(),
				BredAnimalsTrigger.TriggerInstance.bredAnimals(
					Optional.of(EntityPredicate.Builder.entity().of(entityTypes, animal).build()),
					Optional.of(EntityPredicate.Builder.entity().of(entityTypes, animal).build()),
					Optional.empty()
				)
			)
	...
	private static Advancement.Builder addFish(final Advancement.Builder advancement, final HolderGetter<Item> items) {
		for (Item fish : FISH) {
			advancement.addCriterion(
				...
				FishingRodHookedTrigger.TriggerInstance.fishedItem(Optional.empty(), Optional.empty(), Optional.of(ItemPredicate.Builder.item().of(items, fish).build()))
			);
		}
