...
public class PlayerTabOverlay {
	...
	private static final Comparator<PlayerInfo> PLAYER_COMPARATOR = Comparator.comparingInt(p -> -p.getTabListOrder())
	...
	public void extractRenderState(
		...
		if (!this.healthStates.isEmpty()) {
			Set<UUID> playerIds = (Set<UUID>)playerInfos.stream().map(player -> player.getProfile().id()).collect(Collectors.toSet());
		...
		for (int i = 0; i < slots; i++) {
			...
			if (i < playerInfos.size()) {
				...
				PlayerTabOverlay.ScoreDisplayEntry displayInfo = (PlayerTabOverlay.ScoreDisplayEntry)entriesToDisplay.get(i);
	...
	private void extractTablistHearts(final int yo, final int left, final int right, final UUID profileId, final GuiGraphicsExtractor graphics, final int score) {
		PlayerTabOverlay.HealthState health = (PlayerTabOverlay.HealthState)this.healthStates
			.computeIfAbsent(profileId, id -> new PlayerTabOverlay.HealthState(score));
