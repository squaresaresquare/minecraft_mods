...
public class SkinManager {
		...
		return CompletableFuture.allOf(skinTexture, capeTexture, elytraTexture)
			.thenApply(
				unused -> new PlayerSkin(
					(ClientAsset.Texture)skinTexture.join(),
					(ClientAsset.Texture)capeTexture.join(),
					(ClientAsset.Texture)elytraTexture.join(),
					model,
	...
	private class TextureCache {
		...
		public CompletableFuture<ClientAsset.Texture> getOrLoad(final MinecraftProfileTexture texture) {
			...
			CompletableFuture<ClientAsset.Texture> future = (CompletableFuture<ClientAsset.Texture>)this.textures.get(hash);
			if (future == null) {
