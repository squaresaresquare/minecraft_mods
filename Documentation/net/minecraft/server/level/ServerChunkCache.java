...
public class ServerChunkCache extends ChunkSource {
	...
	public ChunkAccess getChunk(final int x, final int z, final ChunkStatus targetStatus, final boolean loadOrGenerate) {
		if (Thread.currentThread() != this.mainThread) {
			...
			ChunkResult<ChunkAccess> chunkResult = (ChunkResult<ChunkAccess>)serverFuture.join();
	...
	public CompletableFuture<ChunkResult<ChunkAccess>> getChunkFuture(final int x, final int z, final ChunkStatus targetStatus, final boolean loadOrGenerate) {
		...
		if (isMainThread) {
			...
			serverFuture = CompletableFuture.supplyAsync(() -> this.getChunkFutureMainThread(x, z, targetStatus, loadOrGenerate), this.mainThreadProcessor)
