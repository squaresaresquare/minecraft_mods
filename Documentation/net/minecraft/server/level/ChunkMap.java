...
public class ChunkMap extends SimpleRegionStorage implements ChunkHolder.PlayerProvider, GeneratingChunkMap {
	...
	public CompletableFuture<ChunkAccess> applyStep(
		...
		if (step.targetStatus() == ChunkStatus.EMPTY) {
			...
			try {
				...
				category.setDetail("Status being generated", (CrashReportDetail<String>)(() -> step.targetStatus().getName()));
	...
	private boolean isExistingChunkFull(final ChunkPos pos) {
		...
		if (cachedChunkType != 0) {
			...
			try {
				currentTag = (CompoundTag)((Optional)this.readChunk(pos).join()).orElse(null);
				if (currentTag == null) {
					this.markPositionReplaceable(pos);
					return false;
				}
	...
	void dumpChunks(final Writer output) throws IOException {
		...
		for (Entry<ChunkHolder> entry : this.visibleChunkMap.long2ObjectEntrySet()) {
			...
			csvOutput.writeRow(
				...
				fullChunk.map(c -> c.getBlockEntities().size()).orElse(0),
				this.ticketStorage.getTicketDebugString(posKey, true),
				this.distanceManager.getChunkLevel(posKey, true),
				fullChunk.map(levelChunk -> levelChunk.getBlockTicks().count()).orElse(0),
				fullChunk.map(levelChunk -> levelChunk.getFluidTicks().count()).orElse(0)
			);
		}
	}
	...
	private static String printFuture(final CompletableFuture<ChunkResult<LevelChunk>> future) {
		try {
			ChunkResult<LevelChunk> result = (ChunkResult<LevelChunk>)future.getNow(null);
			if (result != null) {
	...
	protected void tick() {
		...
		List<ServerPlayer> movedPlayers = Lists.<ServerPlayer>newArrayList();
