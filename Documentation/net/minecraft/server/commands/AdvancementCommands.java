		...
		if (count == 0) {
				...
				if (players.size() == 1) {
					throw ERROR_NO_ACTION_PERFORMED.create(
						Component.translatable(
							...
							((ServerPlayer)players.iterator().next()).getDisplayName()
						)
					);
				} else {
					throw ERROR_NO_ACTION_PERFORMED.create(
						Component.translatable(action.getKey() + ".one.to.many.failure", Advancement.name((AdvancementHolder)advancements.iterator().next()), players.size())
					);
				}
			} else if (players.size() == 1) {
				throw ERROR_NO_ACTION_PERFORMED.create(
					Component.translatable(action.getKey() + ".many.to.one.failure", advancements.size(), ((ServerPlayer)players.iterator().next()).getDisplayName())
				);
			} else {
			...
			if (advancements.size() == 1) {
				if (players.size() == 1) {
					...
					source.sendSuccess(
						() -> Component.translatable(
							action.getKey() + ".one.to.many.success", Advancement.name((AdvancementHolder)advancements.iterator().next()), players.size()
						),
						true
					);
				}
				...
				source.sendSuccess(
					() -> Component.translatable(action.getKey() + ".many.to.one.success", advancements.size(), ((ServerPlayer)players.iterator().next()).getDisplayName()),
					true
				);
			} else {
		...
		if (!advancement.criteria().containsKey(criterion)) {
			...
			if (count == 0) {
				if (players.size() == 1) {
					throw ERROR_NO_ACTION_PERFORMED.create(
						Component.translatable(
							action.getKey() + ".criterion.to.one.failure", criterion, Advancement.name(holder), ((ServerPlayer)players.iterator().next()).getDisplayName()
						)
					);
				} else {
					throw ERROR_NO_ACTION_PERFORMED.create(
				...
				if (players.size() == 1) {
					source.sendSuccess(
						() -> Component.translatable(
							action.getKey() + ".criterion.to.one.success", criterion, Advancement.name(holder), ((ServerPlayer)players.iterator().next()).getDisplayName()
						),
						true
					);
				} else {
