...
public class ExperienceCommand {
	...
	private static int addExperience(
		...
		if (players.size() == 1) {
			source.sendSuccess(
				() -> Component.translatable("commands.experience.add." + type.name + ".success.single", amount, ((ServerPlayer)players.iterator().next()).getDisplayName()),
				true
			);
		} else {
			...
			if (players.size() == 1) {
				source.sendSuccess(
					() -> Component.translatable(
						"commands.experience.set." + type.name + ".success.single", amount, ((ServerPlayer)players.iterator().next()).getDisplayName()
					),
					true
				);
			} else {
