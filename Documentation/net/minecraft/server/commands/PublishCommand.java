					...
					Commands.argument("allowCommands", BoolArgumentType.bool())
						...
						.then(
							Commands.argument("gamemode", GameModeArgument.gameMode())
								.executes(
									c -> publish(c.getSource(), HttpUtil.getAvailablePort(), BoolArgumentType.getBool(c, "allowCommands"), GameModeArgument.getGameMode(c, "gamemode"))
								)
								.then(
									Commands.argument("port", IntegerArgumentType.integer(0, 65535))
										.executes(
											c -> publish(
												c.getSource(), IntegerArgumentType.getInteger(c, "port"), BoolArgumentType.getBool(c, "allowCommands"), GameModeArgument.getGameMode(c, "gamemode")
											)
										)
								)
						)
