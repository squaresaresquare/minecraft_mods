...
public class SpawnArmorTrimsCommand {
	...
	private static int spawnArmorTrim(final CommandSourceStack source, final Player player, final ResourceKey<TrimPattern> pattern) {
		return spawnArmorTrims(
			source, player, Stream.of((Holder.Reference)source.getServer().registryAccess().lookupOrThrow(Registries.TRIM_PATTERN).get(pattern).orElseThrow())
		);
	}
	...
	private static int spawnArmorTrims(final CommandSourceStack source, final Player player, final Stream<Holder.Reference<TrimPattern>> patterns) {
		...
		for (int materialIndex = 0; materialIndex < sortedMaterials.size(); materialIndex++) {
			Holder.Reference<TrimMaterial> material = (Holder.Reference<TrimMaterial>)sortedMaterials.get(materialIndex);

			for (int patternIndex = 0; patternIndex < sortedPatterns.size(); patternIndex++) {
				Holder.Reference<TrimPattern> pattern = (Holder.Reference<TrimPattern>)sortedPatterns.get(patternIndex);
				ArmorTrim trim = new ArmorTrim(material, pattern);

				for (int itemIndex = 0; itemIndex < equippableItems.size(); itemIndex++) {
					Holder.Reference<Item> equippableItem = (Holder.Reference<Item>)equippableItems.get(itemIndex);
					...
					Equippable equippable = (Equippable)Objects.requireNonNull(stack.get(DataComponents.EQUIPPABLE));
