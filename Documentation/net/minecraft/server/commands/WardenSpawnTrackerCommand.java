...
public class WardenSpawnTrackerCommand {
	...
	private static int setWarningLevel(final CommandSourceStack source, final Collection<? extends Player> players, final int warningLevel) {
		...
		if (players.size() == 1) {
			source.sendSuccess(
				() -> Component.translatable("commands.warden_spawn_tracker.set.success.single", ((Player)players.iterator().next()).getDisplayName()), true
			);
		} else {
	...
	private static int resetTracker(final CommandSourceStack source, final Collection<? extends Player> players) {
		...
		if (players.size() == 1) {
			source.sendSuccess(
				() -> Component.translatable("commands.warden_spawn_tracker.clear.success.single", ((Player)players.iterator().next()).getDisplayName()), true
			);
		} else {
