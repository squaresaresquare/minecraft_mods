									...
									Commands.literal("color")
										.then(
											Commands.argument("color", ColorArgument.color())
												.executes(c -> setWaypointColor(c.getSource(), WaypointArgument.getWaypoint(c, "waypoint"), ColorArgument.getColor(c, "color")))
										)
										.then(
								...
								return livingEntity.getFeedbackDisplayName()
									...
									.withStyle(
										s -> s.withClickEvent(new ClickEvent.SuggestCommand("/execute in " + dimension + " run tp @s " + pos.getX() + " " + pos.getY() + " " + pos.getZ()))
											...
											.withColor((Integer)transmitter.waypointIcon().color.orElse(-1))
									);
							} else {
