	...
	private static int clearInventory(
		...
		if (count == 0) {
			...
			if (maxCount == 0) {
				if (players.size() == 1) {
					source.sendSuccess(
						() -> Component.translatable("commands.clear.test.single", finalCount, ((ServerPlayer)players.iterator().next()).getDisplayName()), true
					);
				} else {
				...
				source.sendSuccess(
					() -> Component.translatable("commands.clear.success.single", finalCount, ((ServerPlayer)players.iterator().next()).getDisplayName()), true
				);
			} else {
