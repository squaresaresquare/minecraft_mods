		...
		public static MessageArgument.Message parseText(final StringReader reader, final boolean allowSelectors) throws CommandSyntaxException {
			if (reader.getRemainingLength() > 256) {
				...
				if (!allowSelectors) {
					...
					List<MessageArgument.Part> result = Lists.<MessageArgument.Part>newArrayList();
					...
					while (true) {
						...
						while (true) {
							if (!reader.canRead()) {
								return new MessageArgument.Message(text, (MessageArgument.Part[])result.toArray(new MessageArgument.Part[0]));
							}
