...
public class ResourceKeyArgument<T> implements ArgumentType<ResourceKey<T>> {
	...
	public static <T> ResourceKey<T> getRegistryKey(
		...
		return (ResourceKey<T>)value.orElseThrow(() -> exceptionType.create(argument.identifier()));
	}
	...
	private static <T> Holder.Reference<T> resolveKey(
		...
		return (Holder.Reference<T>)getRegistry(context, registryKey).get(key).orElseThrow(() -> exception.create(key.identifier()));
	}
