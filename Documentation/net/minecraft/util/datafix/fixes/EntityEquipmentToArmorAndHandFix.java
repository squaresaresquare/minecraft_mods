				...
				this.fixTypeEverywhere(
					...
					ops -> {
						...
						return named -> named.mapSecond(equipmentField -> {
							...
							if (!items.isEmpty()) {
								handItems = Either.left(Lists.<ItemStackNew>newArrayList((ItemStackNew[])(new Object[]{items.getFirst(), emptyStack})));
							}

							if (items.size() > 1) {
								List<ItemStackNew> armor = Lists.<ItemStackNew>newArrayList(emptyStack, emptyStack, emptyStack, emptyStack);
	...
	private static Dynamic<?> fixDropChances(Dynamic<?> tag) {
		...
		if (dropChances.isPresent()) {
			...
			if (tag.get("ArmorDropChances").result().isEmpty()) {
				tag = tag.set(
					"ArmorDropChances",
					tag.createList(Stream.of((Float)chances.next(), (Float)chances.next(), (Float)chances.next(), (Float)chances.next()).map(tag::createFloat))
				);
			}
		}
