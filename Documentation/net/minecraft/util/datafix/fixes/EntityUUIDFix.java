...
public class EntityUUIDFix extends AbstractUUIDFix {
	...
	private static Dynamic<?> updatePiglin(final Dynamic<?> tag) {
		return tag.update(
			"Brain",
			brain -> brain.update(
				"memories", memories -> memories.update("minecraft:angry_at", angryAt -> (Dynamic)replaceUUIDString(angryAt, "value", "value").orElseGet(() -> {
	...
	private static Dynamic<?> updateFox(final Dynamic<?> tag) {
		Optional<Dynamic<?>> trustedUUIDs = tag.get("TrustedUUIDs")
			.result()
			.map(uuidTags -> tag.createList(uuidTags.asStream().map(uuidTag -> (Dynamic)createUUIDFromML(uuidTag).orElseGet(() -> {
				LOGGER.warn("Trusted contained invalid data.");
				return uuidTag;
			}))));
		return DataFixUtils.orElse(trustedUUIDs.map(trusted -> tag.remove("TrustedUUIDs").set("Trusted", trusted)), tag);
	}
				...
				attributes.asStream()
					.map(
						attribute -> attribute.update(
							"Modifiers",
							modifiers -> attribute.createList(modifiers.asStream().map(modifier -> (Dynamic)replaceUUIDLeastMost(modifier, "UUID", "UUID").orElse(modifier)))
						)
					)
			)
		);
	...
	private static Dynamic<?> updateProjectile(final Dynamic<?> tag) {
		return DataFixUtils.orElse(tag.get("OwnerUUID").result().map(owner -> tag.remove("OwnerUUID").set("Owner", owner)), tag);
	}
