	...
	public Dynamic<?> fixTag(final Dynamic<Tag> input) {
		...
		if (normalConfig.isEmpty()) {
			...
			if (ominousConfig.isEmpty()) {
				...
				Identifier registryLocation = (Identifier)TrialSpawnerConfigInRegistryFix.VanillaTrialChambers.CONFIGS_TO_KEY
					.get(Pair.of((Dynamic)normalConfig.get(), (Dynamic)ominousConfig.get()));
