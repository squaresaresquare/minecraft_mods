...
public class WorldGenSettingsFix extends DataFix {
	...
	private static <T> Dynamic<T> fix(final Dynamic<T> input) {
		...
		Optional<String> name = input.get("generatorName").asString().<String>map(n -> n.toLowerCase(Locale.ROOT)).result();
		Optional<String> legacyCustomOptions = (Optional<String>)input.get("legacy_custom_options")
			.asString()
			.result()
			.map(Optional::of)
		...
		if (name.equals(Optional.of("customized"))) {
			...
			String generateMapFeatures = (String)name.get();
			switch (generateMapFeatures) {
						...
						ImmutableMap.of(
							...
							input.createMap(
								ImmutableMap.of(
									...
									(Dynamic<?>)flatSettings.get("layers")
										.result()
										.orElseGet(
											() -> input.createList(
												Stream.of(
				...
				case "buffet":
					...
					Dynamic<T> biomeSource = (Dynamic<T>)settings.get("biome_source")
						.result()
	...
	private static <T> Map<Dynamic<T>, Dynamic<T>> fixFlatStructures(final DynamicOps<T> ops, final OptionalDynamic<T> settings) {
		...
		Map<String, WorldGenSettingsFix.StructureFeatureConfiguration> structureConfig = Maps.<String, WorldGenSettingsFix.StructureFeatureConfiguration>newHashMap();
										...
										switch (optionName) {
											...
											case "separation":
												if ("oceanmonument".equals(structureName)) {
													WorldGenSettingsFix.StructureFeatureConfiguration config = (WorldGenSettingsFix.StructureFeatureConfiguration)structureConfig.getOrDefault(
		...
		structureBuilder.put(
			...
			settings.createMap(
				(Map<? extends Dynamic<?>, ? extends Dynamic<?>>)structureConfig.entrySet()
					.stream()
					.collect(
						Collectors.toMap(e -> settings.createString((String)e.getKey()), e -> ((WorldGenSettingsFix.StructureFeatureConfiguration)e.getValue()).serialize(ops))
					)
			)
		);
