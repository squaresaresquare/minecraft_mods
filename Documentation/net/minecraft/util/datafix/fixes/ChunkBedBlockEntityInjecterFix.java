		...
		return TypeRewriteRule.seq(
			...
			this.fixTypeEverywhereTyped(
				...
				input -> {
					...
					List<TE> tileEntities = Lists.<TE>newArrayList(level.getOrCreate(tileEntitiesF));
					...
					for (Dynamic<?> sectionTag : levelTag.get("Sections").asList(Function.identity())) {
						...
						Streams.mapWithIndex(sectionTag.get("Blocks").asIntStream(), (block, index) -> {
								if (416 == (block & 0xFF) << 4) {
									...
									int yy = p >> 8 & 15;
									int zz = p >> 4 & 15;
									Map<Dynamic<?>, Dynamic<?>> bedTag = Maps.<Dynamic<?>, Dynamic<?>>newHashMap();
							...
							.forEachOrdered(
								bedTag -> {
									if (bedTag != null) {
										tileEntities.add(
											((Pair)tileEntityType.read(sectionTag.createMap(bedTag))
													.result()
