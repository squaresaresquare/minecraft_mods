	...
	private Dynamic<?> fixPlayer(Dynamic<?> tag) {
		...
		if (spawnX.isPresent() && spawnY.isPresent() && spawnZ.isPresent()) {
			Dynamic<?> respawn = tag.createMap(
				Map.of(
					tag.createString("pos"),
					ExtraDataFixUtils.createBlockPos(tag, ((Number)spawnX.get()).intValue(), ((Number)spawnY.get()).intValue(), ((Number)spawnZ.get()).intValue())
				)
			);
		...
		if (enteredNetherPos.isPresent()) {
			tag = tag.remove("enteredNetherPosition")
				.set(
					...
					tag.createList(
						Stream.of(
							tag.createDouble(((Dynamic)enteredNetherPos.get()).get("x").asDouble(0.0)),
							tag.createDouble(((Dynamic)enteredNetherPos.get()).get("y").asDouble(0.0)),
							tag.createDouble(((Dynamic)enteredNetherPos.get()).get("z").asDouble(0.0))
						)
					)
				);
		}
