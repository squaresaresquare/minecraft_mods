...
public class ItemStackComponentizationFix extends DataFix {
	...
	private static void fixItemStack(final ItemStackComponentizationFix.ItemStackData itemStack, final Dynamic<?> dynamic) {
		...
		if (itemStack.is("minecraft:filled_map")) {
			...
			Map<? extends Dynamic<?>, ? extends Dynamic<?>> decorations = (Map<? extends Dynamic<?>, ? extends Dynamic<?>>)itemStack.removeTag("Decorations")
				.asStream()
	...
	private static Dynamic<?> fixBlockStateTag(final Dynamic<?> blockStateTag) {
		return DataFixUtils.orElse(blockStateTag.asMapOpt().result().map(entries -> (Map)entries.collect(Collectors.toMap(Pair::getFirst, entry -> {
	...
	private static Dynamic<?> fixDisplay(final ItemStackComponentizationFix.ItemStackData itemStack, Dynamic<?> display, final int hideFlags) {
		display.get("Name")
			...
			.ifPresent(name -> itemStack.setComponent("minecraft:custom_name", name));
			...
			case "minecraft:decorated_pot" -> {
				...
				if (item.isPresent()) {
					itemStack.setComponent(
						"minecraft:container",
						blockEntity.createList(Stream.of(blockEntity.emptyMap().set("slot", blockEntity.createInt(0)).set("item", (Dynamic<?>)item.get())))
					);
				}
	...
	private static Optional<Pair<String, Integer>> parseEnchantment(final Dynamic<?> entry) {
		return entry.get("id")
			.asString()
			.<Number, Pair<String, Integer>>apply2stable((id, level) -> Pair.of(id, Mth.clamp(level.intValue(), 0, 255)), entry.get("lvl").asNumber())
			.result();
	}
		...
		if (!oldPredicate.isEmpty()) {
			Dynamic<?> component = dynamic.emptyMap()
				.set(
					"predicates",
					dynamic.createList(
						((Dynamic)oldPredicate.get())
							.asStream()
							.map(value -> DataFixUtils.orElse(value.asString().map(string -> fixBlockStatePredicate(value, string)).result(), value))
					)
				);
			if (hideInTooltip) {
				component = component.set("show_in_tooltip", dynamic.createBoolean(false));
							...
							.map(
								pair -> {
									Dynamic<?> property = dynamic.emptyMap()
										.set("name", dynamic.createString((String)entry.getKey()))
	...
	private static class ItemStackData {
		...
		private static <T> Dynamic<T> mergeRemainder(final Dynamic<T> itemStack, final Dynamic<?> remainder) {
			...
			return (Dynamic<T>)ops.getMap(itemStack.getValue())
