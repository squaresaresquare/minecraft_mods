...
public class StatsRenameFix extends DataFix {
		...
		if (customFieldType == null) {
			...
			return this.fixTypeEverywhereTyped(
				...
				input -> input.updateTyped(
					criteriaTypeFinder,
					stats -> stats.updateTyped(customTypeFinder, custom -> custom.update(idFinder, value -> (String)this.renames.getOrDefault(value, value)))
				)
			);
		}
	}
	...
	private TypeRewriteRule createStatRule() {
		...
		return this.fixTypeEverywhereTyped(
			...
			outputType,
			input -> input.updateTyped(
				statsFinder, stats -> stats.updateTyped(customFinder, custom -> custom.update(nameFinder, value -> (String)this.renames.getOrDefault(value, value)))
			)
		);
	}
}
