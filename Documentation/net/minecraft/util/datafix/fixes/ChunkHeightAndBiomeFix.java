...
public class ChunkHeightAndBiomeFix extends DataFix {
		...
		return this.fixTypeEverywhereTyped(
			...
			chunk -> chunk.updateTyped(
				...
				level -> {
					...
					Set<String> blocksInChunk = Sets.<String>newHashSet();
					...
					level = level.updateTyped(sectionsFinder, sectionsType, sections -> {
						...
						List<Dynamic<?>> sectionsList = (List<Dynamic<?>>)dynamic.asStream().map(sectionx -> {
	...
	private static Dynamic<?>[] getBiomeContainers(
		...
		int[] oldBiomes = (int[])tag.get("Biomes").asIntStreamOpt().result().map(IntStream::toArray).orElse(null);
	...
	private static Dynamic<?> addPaddingEntries(final Dynamic<?> chunkTag, final String key) {
		List<Dynamic<?>> list = (List<Dynamic<?>>)chunkTag.get(key).orElseEmptyList().asStream().collect(Collectors.toCollection(ArrayList::new));
		if (list.size() == 24) {
			return chunkTag;
		} else {
	...
	private static Dynamic<?> makeOptimizedPalettedContainer(Dynamic<?> palette, final Dynamic<?> data) {
		List<Dynamic<?>> paletteList = (List<Dynamic<?>>)palette.asStream().collect(Collectors.toCollection(ArrayList::new));
