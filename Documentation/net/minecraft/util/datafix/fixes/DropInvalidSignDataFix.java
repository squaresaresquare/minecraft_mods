...
public class DropInvalidSignDataFix extends DataFix {
	...
	private static <T> Dynamic<T> fixText(final Dynamic<T> tag) {
		...
		if (filteredLines.isEmpty()) {
			...
			List<Dynamic<T>> newFilteredLines = Streams.<Dynamic, Dynamic<T>>mapWithIndex((Stream<Dynamic>)filteredLines.get(), (line, index) -> {
				Dynamic<T> fallbackLine = index < lines.size() ? (Dynamic)lines.get((int)index) : emptyComponent;
