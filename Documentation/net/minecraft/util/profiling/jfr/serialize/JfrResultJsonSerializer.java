...
public class JfrResultJsonSerializer {
	...
	private JsonElement structureGen(final List<StructureGenStat> structureGenStats) {
		...
		if (optionalSummary.isEmpty()) {
			...
			TimedStatSummary<StructureGenStat> summary = (TimedStatSummary<StructureGenStat>)optionalSummary.get();
			JsonArray structureJsonArray = new JsonArray();
			root.add("structure", structureJsonArray);
			((Map)structureGenStats.stream().collect(Collectors.groupingBy(StructureGenStat::structureName)))
				.forEach(
					(structureName, timedStat) -> {
						...
						if (!optionalStatSummary.isEmpty()) {
							TimedStatSummary<StructureGenStat> statSummary = (TimedStatSummary<StructureGenStat>)optionalStatSummary.get();
							...
							root.add(
								"secondSlowest",
								(JsonElement)(summary.secondSlowest() != null ? (JsonElement)structureGenStatJsonGenerator.apply(summary.secondSlowest()) : JsonNull.INSTANCE)
							);
						}
					}
				);
	...
	private JsonElement chunkGen(final List<Pair<ChunkStatus, TimedStatSummary<ChunkGenStat>>> chunkGenSummary) {
		...
		if (chunkGenSummary.isEmpty()) {
			...
			for (Pair<ChunkStatus, TimedStatSummary<ChunkGenStat>> summaryByStatus : chunkGenSummary) {
				...
				chunkStatusJson.add(
					"secondSlowest",
					(JsonElement)(chunkStat.secondSlowest() != null ? (JsonElement)chunkGenStatJsonGenerator.apply(chunkStat.secondSlowest()) : JsonNull.INSTANCE)
				);
			}
