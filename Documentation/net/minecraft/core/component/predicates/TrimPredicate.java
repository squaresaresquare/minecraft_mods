...
public record TrimPredicate(Optional<HolderSet<TrimMaterial>> material, Optional<HolderSet<TrimPattern>> pattern)
	...
	public boolean matches(final ArmorTrim value) {
		return this.material.isPresent() && !((HolderSet)this.material.get()).contains(value.material())
			? false
			: !this.pattern.isPresent() || ((HolderSet)this.pattern.get()).contains(value.pattern());
	}
}
