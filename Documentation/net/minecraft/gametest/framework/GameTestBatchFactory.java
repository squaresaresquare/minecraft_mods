...
public class GameTestBatchFactory {
	...
	public static List<GameTestBatch> divideIntoBatches(
		...
		Map<Holder<TestEnvironmentDefinition<?>>, List<GameTestInfo>> testsPerBatch = (Map<Holder<TestEnvironmentDefinition<?>>, List<GameTestInfo>>)allTests.stream()
			.flatMap(test -> decorator.decorate(test, level))
	...
	public static GameTestRunner.GameTestBatcher fromGameTestInfo(final int maxTestsPerBatch) {
		return gameTestInfos -> {
			Map<Holder<TestEnvironmentDefinition<?>>, List<GameTestInfo>> testFunctionsPerBatch = (Map<Holder<TestEnvironmentDefinition<?>>, List<GameTestInfo>>)gameTestInfos.stream()
