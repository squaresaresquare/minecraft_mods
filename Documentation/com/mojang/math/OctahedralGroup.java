...
public enum OctahedralGroup implements StringRepresentable {
	...
	private static final OctahedralGroup[][] CAYLEY_TABLE = Util.make(() -> {
		...
		Map<Integer, OctahedralGroup> fingerprints = (Map<Integer, OctahedralGroup>)Arrays.stream(values).collect(Collectors.toMap(OctahedralGroup::trace, o -> o));
	...
	private static final OctahedralGroup[] INVERSE_TABLE = (OctahedralGroup[])Arrays.stream(values())
		.map(f -> (OctahedralGroup)Arrays.stream(values()).filter(s -> f.compose(s) == IDENTITY).findAny().get())
