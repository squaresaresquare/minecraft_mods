...
@[code transcludeWith=:::spawn_egg](@/reference/1.21.11/src/main/java/com/example/docs/item/ModItems.java)
...
@[code](@/reference/1.21.11/src/main/generated/assets/example-mod/models/item/custom_spawn_egg.json)

## 创建客户端物品 {#creating-the-client-item}

在 `assets/example-mod/items` 目录中创建客户端物品 JSON，文件名与物品模型的 ID 相同：`custom_spawn_egg.json`。

@[code](@/reference/1.21.11/src/main/generated/assets/example-mod/items/custom_spawn_egg.json)
